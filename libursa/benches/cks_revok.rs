...
fn get_credential_secret_value(master_secret: &MasterSecret) -> CredentialValues {
...
)
{
    ...
    let (rev_key_pub, rev_key_priv, rev_reg, mut rev_tails_generator) =
...
fn get_blinded_credential(
    ...
    let credential_secret_value = get_credential_values(&Prover::new_master_secret().unwrap());
...
fn gen_issuance(
    ...
    blinding_correctness_nonce: &Nonce,
    signature_correctness_nonce: &Nonce,
    ...
    let (credential_signature, signature_correctness_proof, rr_delta) =
...
fn setup_cred_and_issue(
    ...
    let (
        ...
        rev_key_priv,
        simple_tail_accessor,
    ) = setup_cred(max_cred_num, issuance_by_default);
    ...
    for i in 0..max_cred_num {
        ...
        // 8. Issuer creates and sign credential values
        let rev_idx = i + 1;
        let (mut credential_signature, signature_correctness_proof, rr_delta) = gen_issuance(
            &blinded_credential_secrets,
            &blinded_credential_secrets_correctness_proof,
            &blinding_correctness_nonce,
            &signature_correctness_nonce,
...
fn bench_cks_issuance_revok_on_demand_issuance(c: &mut Criterion) {
    c.bench_function_over_inputs(
        "cks revocation issuance generation with on demand issuance",
        move |b, (max_cred_num, _num_proofs_to_do)| {
            let max_cred_num = *max_cred_num as u32;
            let issuance_by_default = false;

            let (
                _credential_schema,
                _non_credential_schema,
                credential_pub_key,
                credential_priv_key,
                credential_key_correctness_proof,
                mut rev_reg,
            ...
            let (
                blinded_credential_secrets,
                _credential_secrets_blinding_factors,
                blinded_credential_secrets_correctness_proof,
                blinding_correctness_nonce,
                credential_values,
            ) = get_blinded_credential(
...
criterion_group! {
    name = cks_issuance_revok_on_demand;
    config = Criterion::default().sample_size(3);
    //config = Criterion::default();
    targets =
        bench_cks_issuance_revok_on_demand_issuance,
}
...
criterion_main!(cks_issuance_revok_on_demand, cks_prove_revok_on_demand, cks_verify_revok_on_demand);
